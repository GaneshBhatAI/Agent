...
export const Repositories: React.FC = () => {
  ...
  const fetchRepositories = async () => {
    ...
    try {
      ...
      if (repoRes.data.length > 0) {
        if (!selectedRepo || !repoRes.data.find((r: any) => r.id === selectedRepo.id)) {
          setSelectedRepo(repoRes.data[0]);
          setSelectedBranch(repoRes.data[0].default_branch || 'master');
        }
      } else {
    ...
    fetch(`https://api.github.com/repos/${owner}/${name}/git/trees/${branch}?recursive=1`, { headers })
      ...
      .then((data) => {
        if (data && data.tree && Array.isArray(data.tree)) {
          ...
          // If public without tree endpoint or fallback
  ...
  const handleOpenFile = async (fileNode: TreeNode) => {
    ...
    // Fetch live content from GitHub API
    if (selectedRepo) {
      const owner = selectedRepo.github_owner || selectedRepo.owner || 'GaneshBhatAI';
      const name = selectedRepo.repository_name || selectedRepo.name || 'Agent';
      const branch = selectedBranch || selectedRepo.default_branch || 'master';
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium">
            Connect your GitHub account with PAT • Select active repositories • Inspect live code & dispatch bots
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          {repos.length > 0 && (
            <>
                ...
                >
                  {repos.map((r) => (
                    <option key={r.id} value={r.id}>
                      {r.github_owner}/{r.repository_name}
                    </option>
                  ))}
                </select>
              </div>

              {/* Edit / Manage Repositories */}
              <button
                onClick={() => setIsConnectModalOpen(true)}
                title="Edit and manage connected repositories"
                className="flex items-center gap-1.5 rounded-full border border-purple-200 bg-white px-3.5 py-1.5 text-xs font-bold text-slate-700 hover:bg-purple-50 shadow-2xs cursor-pointer"
              >
              ...
              <button
                ...
                title="Disconnect selected repository"
                className="rounded-full border border-purple-200 bg-white p-2 text-slate-400 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 transition-colors cursor-pointer shadow-2xs"
              >
          ...
          >
            ...
            <span>{repos.length === 0 ? 'Connect GitHub (PAT)' : 'Add More Repos'}</span>
          </button>
        </div>
      </div>

      {/* Main Bot Explorer Content or Clean Empty State */}
      {repos.length === 0 ? (
        /* Empty State when no repository is connected yet */
        ...
        /* Explorer Grid when user has connected repositories */
